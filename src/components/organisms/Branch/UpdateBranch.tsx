...
const UpdateBranch: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProvinces = async () => {
            try {
                const response = await fetch('https://settingsbanquitoapp-kjduy-dev.apps.sandbox-m3.1530.p1.openshiftapps.com/api/location/provinces')
    ...
    useEffect(() => {
        const fetchCantons = async () => {
            try {
                if (selectedProvince) {
                    const response = await fetch(`https://settingsbanquitoapp-kjduy-dev.apps.sandbox-m3.1530.p1.openshiftapps.com/api/location/province/${selectedProvince}`)
                    const data = await response.json()
                    setCantonsData(data)
                }
            } catch (error) {
    ...
    useEffect(() => {
        const fetchParishes = async () => {
            try {
                if (selectedCanton) {
                    const response = await fetch(`https://settingsbanquitoapp-kjduy-dev.apps.sandbox-m3.1530.p1.openshiftapps.com/api/location/canton/${selectedCanton}`)
                    const data = await response.json()
